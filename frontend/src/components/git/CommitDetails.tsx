...
import { Loader2, Copy, Check, FileText, FilePlus, FileMinus, FileEdit } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    async function fetchDetails() {
      try {
        ...
        const result = await fetchCommitDetails(repoPath, hash);
        if (!cancelled) {
          setData(result.data);
          setLoading(false);
        }
    ...
    >
      ...
      {(data.author || formattedDate) && (
        <div className="text-xs mb-3" style={{ color: 'var(--text-secondary)' }}>
          ...
          {data.author && formattedDate && <span className="mx-2">&bull;</span>}
