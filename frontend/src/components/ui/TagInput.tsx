    ...
    <div>
      ...
      >
        {value.map((tag, i) => (
          <span
            key={`${tag}-${i}`}
            className="inline-flex items-center gap-1 rounded-md bg-[var(--bg-primary)] border border-[var(--border-secondary)] px-2 py-0.5 text-xs text-[var(--text-primary)]"
          >
            {tag}
            <button
              type="button"
        ...
        <input
          ...
          className="min-w-[80px] flex-1 bg-transparent py-0.5 text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none"
        />
      </div>
      {helpText && (
